    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            for j in range(m):
                ...
                R = kn.geometry.angle_axis_to_rotation_matrix(P_6d_flat[0:m-3].view(1,3))
...
class BPnP_fast(torch.autograd.Function):
    ...
    def forward(ctx, pts2d, pts3d, K, ini_pose=None):
        ...
        for i in range(bs):
            pts2d_i_np = np.ascontiguousarray(pts2d[i].detach().cpu()).reshape((n,2))
            if ini_pose is None:              
                # CHANGE: use EPNP instead of PNP_ITERATIVE
                
                _, rvec0, T0, _ = cv.solvePnPRansac(objectPoints=pts3d_np, imagePoints=pts2d_i_np, cameraMatrix=K_np, distCoeffs=None, flags=cv.SOLVEPNP_EPNP, confidence=0.9999 ,reprojectionError=3)
            else:
                rvec0 = np.array(ini_pose[i, 0:3].cpu().view(3, 1))
                T0 = np.array(ini_pose[i, 3:6].cpu().view(3, 1))
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            # We substitute this for a more stable thing
            #inv_J_fy = torch.inverse(J_fy)
            ...
            J_yx = (-1) * torch.linalg.lstsq(J_fy, J_fx).solution
            J_yz = (-1) * torch.linalg.lstsq(J_fy, J_fz).solution
            J_yK = (-1) * torch.linalg.lstsq(J_fy, J_fK).solution

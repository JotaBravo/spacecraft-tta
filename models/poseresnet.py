...
class Decoder(nn.Module):
    ...
    def __init__(self, block, layers, final_layer,inplanes=2048, **kwargs):
...
def get_encoder(num_layers, train=True, **kwargs):

    block_class, layers = resnet_spec[num_layers]

    model = Encoder(block_class, layers, **kwargs)
...
def get_decoder(num_layers, train=True, final_layer=11,inplanes=2048, **kwargs):

    block_class, layers = resnet_spec[num_layers]

    model = Decoder(block_class, layers, final_layer,inplanes, **kwargs)
...
def get_pose_net(num_layers, train=True, **kwargs):
...
def get_mlp(embedding_dim=12,n_hidden_neurons=1024):

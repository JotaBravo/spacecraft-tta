...
tango_loader = loader_lb.TangoVideoLoader(config,config["train_percentage"])
train_loader = DataLoader(tango_loader, batch_size=config["batch_size"], shuffle=shuffle, num_workers=config["num_workers"],drop_last=True)
...
if config["adaptative"]:
    optim_params = [{"params": encoder.parameters(), "lr": config["lr"]},
                    {"params": im_decoder.parameters(), "lr": config["lr"]*config["reduction_lr"]},
                    {"params": mlp_encoder.parameters(), "lr": config["lr"]*config["reduction_lr"]},              
...
for epoch in range(config["epochs"]):
    ...
    for i_iter, data in enumerate(tqdm(train_loader)):
        ...
        data = utils.augmentate_images(data, aug_intensity)
        ...
        rt_tgt     = pnp_fast(p_tgt, kpts_world[0], k_mat_input[0])
        ...
        data["flag_outside"] = flag_tgt

        rt_prev    = pnp_fast(p_prev, kpts_world[0], k_mat_input[0])
        ...
        data["flag_outside_prev"] = flag_prev


        rt_next    = pnp_fast(p_next, kpts_world[0], k_mat_input[0])            
        ...
        data["flag_outside_next"] = flag_next
        ...
        if i_iter % config["save_per_epoch"] == 0:
            ...
            utils.log_metrics_train(writer, data, image_pred_tgt_prev, image_pred_tgt_next,
                                        ...
                                        loss_image_prev, loss_image_next, loss_pnp_sup, loss_pnp_unsup, 
                                        loss_render, torch.tensor(0), loss_unsup, loss_sup, config, i_iter, epoch,
                                        train_loader)
